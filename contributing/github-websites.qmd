...
order: 6
description: |
  A guide on `steno-aarhus` GitHub websites.
---
...
2.  Decide on a name for your project. This should be a short,
    ...
    `on-limit`.
3.  Send your GitHub username and theu name of your project to one of
    the `steno-aarhus` administrators (e.g., Luke Johnston,
    lwjohnst\@gmail.com, or Signe Kirk Brødbæk, signekb\@clin.au.dk).
    Then, we'll set up the website for you on GitHub in a [repository]()
    in the [`steno-aarhus` organisation]().
...
you need to do are [Navigating GitHub](@sec-navigating-github) and
[Adding content](@sec-adding-content-to-website).
...
section below on [Multi-page websites](@sec-multi-page).

## :compass: Navigating GitHub {#sec-navigating-github}

To add content to a GitHub website, you first need to be able to
navigate GitHub and know some of the basics of how GitHub works.
Therefore, we recommend that you go through the material of this
[Introduction to GitHub](https://github-intro.rostools.org/) workshop.
...
## :pencil2: Adding content to a GitHub website {#sec-adding-content-to-website}
...
`.qmd` file in your website's GitHub repository. See [Navigating
GitHub](#sec-navigating-github) for how to do this.
...
add a page called "Om projektet" in a file named `about.qmd`, you need
to add the following three lines to the `navbar` section of your
`_quarto.yml` file:
...
you for example wanted to add a page called "Om os", you would add the
following lines just below the previous lines:
...
and `about-us.qmd` is name of the file with the page content.
...
`pages`, you would write `href: "pages/about.qmd"`.
:::
...
issue,`@lwjohnst86` or `@signekb`. Then, we'll get a notification
and get back to you when we can.